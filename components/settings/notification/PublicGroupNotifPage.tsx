"use client";

import { useState } from "react";
import { motion } from "framer-motion";

export default function PublicGroupNotifPage() {
  return (
    <div className="h-full overflow-y-auto p-4 space-y-4">
      <Section
        title="Public Groups & Channels"
        desc="High-traffic decentralized groups control"
      />

      <Toggle label="Mentions Only (@username)" />
      <Toggle label="Mute Groups with >1000 Members" />
      <Toggle label="Auto-Mute Newly Joined Public Groups" />
      <Toggle label="Highlight Admin Announcements" />

      <div className="pt-4 border-t border-white/10 text-xs text-gray-400">
        Public groups may generate high traffic. These controls help prevent spam
        while keeping important messages visible.
      </div>
    </div>
  );
}

function Section({ title, desc }: any) {
  return (
    <div>
      <h3 className="text-sm font-semibold">{title}</h3>
      <p className="text-xs text-gray-400 mt-1">{desc}</p>
    </div>
  );
}

/* =========================
   CYBER PUBLIC GROUP TOGGLE
========================= */
function Toggle({ label }: any) {
  const [active, setActive] = useState(false);

  return (
    <div className="flex justify-between items-center py-3">
      <span className="text-sm">{label}</span>

      <button
        onClick={() => setActive(!active)}
        className={`relative w-11 h-6 rounded-full border overflow-hidden transition-colors duration-300
          ${
            active
              ? "bg-amber-500/25 border-amber-400/40"
              : "bg-white/10 border-white/10"
          }`}
      >
        {/* warning glow */}
        {active && (
          <div className="absolute inset-0 bg-amber-400/20 blur-md" />
        )}

        {/* traffic scan line */}
        {active && (
          <motion.div
            className="absolute inset-0 bg-linear-to-r from-transparent via-amber-400/30 to-transparent"
            animate={{ x: ["-100%", "100%"] }}
            transition={{ duration: 1.4, repeat: Infinity, ease: "linear" }}
          />
        )}

        {/* knob — centered perfectly */}
        <motion.div
          className={`absolute top-1/2 left-[3px] w-4 h-4 rounded-full -translate-y-1/2 z-10
            ${
              active
                ? "bg-amber-400 shadow-[0_0_10px_#fbbf24]"
                : "bg-gray-400"
            }`}
          animate={{ x: active ? 20 : 0 }}
          transition={{ type: "spring", stiffness: 420, damping: 28 }}
        />
      </button>
    </div>
  );
}
