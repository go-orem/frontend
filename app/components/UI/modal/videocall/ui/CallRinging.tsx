// components/webrtc/ui/CallRinging.tsx
"use client";
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center bg-black/50 backdrop-blur-2xl font-mono text-white">
      ...
      <div className="flex gap-6 mt-4">
        <button onClick={onAccept} className="px-6 py-3 rounded-full bg-green-600">Accept</button>
        <button onClick={() => { setCallState("idle"); onClose?.(); }} className="px-6 py-3 rounded-full bg-red-600">Decline</button>
      </div>
    </div>
  );
}
